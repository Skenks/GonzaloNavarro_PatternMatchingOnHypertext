...
class AlignmentGraph:
    ...
    def topological_order_of_components(self):
        ...
        for i in range(self.node_len()):
            if index[i] == -1:
                self.connect(i, result, index, lowlink, onStack, S)
        result.reverse()
        assert (len(result) > 0);
        ...
        for i in range(self.node_len()):
            assert (belongs_to_component[i] != -1);
            #for neighbor in self.inNeighbors[i]:
                #assert (belongs_to_component[neighbor] <= belongs_to_component[i])
        for i in belongs_to_component:
            print(i)
        return result, belongs_to_component

    def connect(self, node, result, index, lowlink, onStack, S):
        index[node] = self.indexnum
        lowlink[node] = self.indexnum
        self.indexnum += 1
        S.append(node)
        onStack[node] = True
        for neighbor in self.outNeighbors[node]:
            if index[neighbor] == -1: #ako nije vec posjecen (nije bio node u connectu)
                self.connect(neighbor, result, index, lowlink, onStack, S)        #povezi dalje transverzalno
                lowlink[node] = min(lowlink[neighbor], lowlink[node]) #ako neki susjed
            elif onStack[neighbor]: #vec je posjecen prije, ali je trenutno na stogu
                if lowlink[node] < index[neighbor]:
                    print("manji")
                lowlink[node] = min(lowlink[node], index[neighbor]) #tada postoji ciklus

        if lowlink[node] == index[node]:    #ako sam taj cvor ima lowlink svoj index - ili prvi u connect lancu ili nema dalje susjeda
            result.append([])
            while True:
...
class Aligner:
    ...
    def calculate_acyclic(self, graph, current_slice, previous_slice, pattern, j, node):
        start = graph.node_start_in_seq(node)
        end = graph.node_end_in_seq(node)
        current_slice[start] = previous_slice[start] + 1
        char_in_graph = graph.nodeSequences[start]
        match = char_in_graph == pattern[j]
        for neighbor in graph.inNeighbors[node]:
            u = graph.node_end_in_seq(neighbor) - 1  # indeks zadnjeg znaka u susjedu
        ...
        for w in range(start + 1, end):
            char_in_graph = graph.nodeSequences[w]
            match = char_in_graph == pattern[j]
            current_slice[w] = min(current_slice[w - 1] + 1, previous_slice[w] + 1,
                                   previous_slice[w - 1] + (0 if match else 1))

    def calculate_cyclic_component(self, graph, pattern, nodes, current_slice, previous_slice, j):

        for node in nodes:
            self.calculate_acyclic(graph, current_slice, previous_slice, pattern, j, node)

        for node in nodes:
            start = graph.node_start_in_seq(node)
            for neighbor in graph.inNeighbors[node]:
                u = graph.node_start_in_seq(neighbor) - 1
                if current_slice[start] > current_slice[u] + 1:
                    self.recurse_horizontal_scores(graph, self.belongs_to_component, current_slice, node, start, current_slice[u] + 1)
            for w in range(start + 1, graph.node_end_in_seq(node)):
                if current_slice[w] > current_slice[w - 1] + 1:
                    self.recurse_horizontal_scores(graph,current_slice, node, w, current_slice[w - 1] + 1)

    def recurse_horizontal_scores(self, graph, belongs_to_component, current_slice, node, start, new_score):
        ...
        for neighbor in graph.outNeighbors[node]:
            if belongs_to_component[neighbor] == belongs_to_component[node]:
                u = graph.node_start_in_seq(neighbor)
                if current_slice[u] > new_score:
                    self.recurse_horizontal_scores(graph, belongs_to_component, current_slice, neighbor, u, new_score)

    # approximate string(pattern) matching on hypertext(graph)

    def align(self, graph, component_order, pattern):
        #current_slice = np.zeros(graph.sequence_len(), dtype=int)
        #previous_slice = np.zeros(graph.sequence_len(), dtype=int)

        current_slice = [0] * graph.sequence_len()
        previous_slice = [0] * graph.sequence_len()

        # inicijalizacija previous_slicea
        for w in range(len(previous_slice)):
            char_in_graph = graph.get_sequence_char(w)
            previous_slice[w] = 0 if pattern[0] == char_in_graph else 1

        for j in range(1, len(pattern)):
            for i in range(len(component_order)):
                assert (len(component_order[i]) == 1)
                self.calculate_acyclic(graph, current_slice, previous_slice, pattern, j, component_order[i][0])
    ...
    def align_cyclic(self, graph, component_order, pattern):
        current_slice = graph.sequence_len() * [0]
        previous_slice = graph.sequence_len() * [0]
        ...
        for j in range(1, len(pattern)):
            ...
            for component in component_order:
                if len(component) == 1:
                    isAcyclic = True
                    for neighbor in graph.inNeighbors[component[0]]:
                        if neighbor == component[0]:
                            isAcyclic = False
                            break
                    if isAcyclic:
                        self.calculate_acyclic(graph, current_slice, previous_slice, pattern, j, component[0])
                        continue
                self.calculate_cyclic_component(graph, pattern, component, current_slice, previous_slice, j)
            current_slice, previous_slice = previous_slice, current_slice
...
def main():
    ...
    graphfile = os.path.join(current_dir, 'tmp', 'ref10000_linear.gfa')
    ...
    for fastq in fastqs:
        ...
        score = 0
        if isAcyclic:
            score = aligner.align(pattern=fastq.sequence, graph=alignmentGraph, component_order=first)
            scores.append(score)
            print(score)
        else:
            score = aligner.align_cyclic(pattern=fastq.sequence, graph=alignmentGraph, component_order=first)
            scores.append(score)
            print(score)
