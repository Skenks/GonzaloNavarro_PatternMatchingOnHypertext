...
public class Aligner2 {
    ...
    public int alignCycle2(String pattern, List<List<Integer>> stronglyConnectedComponents) {
        List<Integer> realEditDistances = new ArrayList<>(Collections.nCopies(graph.graphSequence.size(), 0));
        List<Integer> iterationEditDistances = new ArrayList<>(Collections.nCopies(graph.graphSequence.size(), 0));
    ...
    public int alignCycle(String pattern, List<List<Integer>> stronglyConnectedComponents) {
        List<Integer> realEditDistances = new ArrayList<>(Collections.nCopies(graph.graphSequence.size(), 0));
        List<Integer> iterationEditDistances = new ArrayList<>(Collections.nCopies(graph.graphSequence.size(), 0));
        ...
        for (int j = 1; j < pattern.length(); j++) {
            ...
            for (var component : stronglyConnectedComponents) {
                if (component.size() == 1) {
                    ...
                    if (isAcyclic) {
                        f(realEditDistances, iterationEditDistances, pattern, j, component.get(0));
                        continue;
                    }
                }
                g(realEditDistances, iterationEditDistances, pattern, j, component);
            }
    ...
    private void g(List<Integer> realEditDistances, List<Integer> iterationEditDistances, String pattern, int j, List<Integer> nodes) {
        for (int node : nodes) {
            f(realEditDistances, iterationEditDistances, pattern, j, node);
        }

        for (int v : nodes) {
            for (int u : graph.inNeighbors.get(v)) {
                propagate(u, v, iterationEditDistances);
            }
        }
    }
    ...
    private void f2(List<Integer> realEditDistances, List<Integer> iterationEditDistances, String pattern, int j, Integer node) {
        int start = graph.getNodeStartInSequence(node);
        int end = graph.getNodeEndInSequence(node);
    ...
    public void propagateIteartion(int u, int v, List<Integer> iterationEditDistances, List<Integer> connectedNodes) {
        int u_end = graph.getNodeEndInSequence(u) - 1;
        int v_start = graph.nodeIndexInGraphSequence.get(v);
        int v_end = graph.getNodeEndInSequence(v) - 1;
        int new_value = iterationEditDistances.get(u_end) + 1;
        if (iterationEditDistances.get(v_start) > new_value) {
            iterationEditDistances.set(v_start, new_value);
            //propagate further inside node
            new_value++;
            for (int c = v_start + 1; c < v_end; c++) {
                if (iterationEditDistances.get(c) > new_value) {
                    iterationEditDistances.set(c, new_value);
                    new_value++;
                } else
                    break;
            }
            for (int z : graph.outNeighbors.get(v)) {
                if (connectedNodes.contains(z)) {
                    propagate(v, z, iterationEditDistances);
                }
            }
        }
    }
